...
public class StartMenuController : MonoBehaviour {
	...
	public void JoinAsClient(EventObject evt = null) {
		...
		if(netManager.IsClientConnected()) {
			Debug.Log("Already connected to " + netManager.client.connection.address);
			return;
		}
...
//		netManager.StartServer();
	...
	public void Disconnect(EventObject evt) {
		if(isHost) {
			...
			netManager.StopClient(); // Seems to work..
//			netManager.client.Disconnect(); // TODO Disconnect properly from server from client (Maybe a cmd needed?)
